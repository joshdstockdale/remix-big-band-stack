...
```
npx create-remix@latest --template joshdstockdale/remix-big-band-stack
```
...
[![Gitpod Ready-to-Code](https://img.shields.io/badge/Gitpod-Ready--to--Code-blue?logo=gitpod)](https://gitpod.io/#https://github.com/joshdstockdale/big-band-stack/tree/main)
...
- Create two apps on Fly, one for staging and one for production:
  ...
  ```sh
  fly apps create big-band-stack-template
  fly apps create big-band-stack-template-staging
  ```

  > **Note:** Once you've successfully created an app, double-check the `fly.toml` file to ensure that the `app` key is the name of the production app you created.
...
- Add a `SESSION_SECRET` to your fly app secrets, to do this you can run the following commands:
  ...
  ```sh
  fly secrets set SESSION_SECRET=$(openssl rand -hex 32) --app big-band-stack-template
  fly secrets set SESSION_SECRET=$(openssl rand -hex 32) --app big-band-stack-template-staging
  ```

  > **Note:** When creating the staging secret, you may get a warning from the Fly CLI that looks like this:
  >
  > ```
  > WARN app flag 'big-band-stack-template-staging' does not match app name in config file 'big-band-stack-template'
  > ```
  >
...
- Create a database for both your staging and production environments. Run the following:
  ...
  ```sh
  fly postgres create --name big-band-stack-template-db
  fly postgres attach --app big-band-stack-template big-band-stack-template-db

  fly postgres create --name big-band-stack-template-staging-db
  fly postgres attach --app big-band-stack-template-staging big-band-stack-template-staging-db
  ```
